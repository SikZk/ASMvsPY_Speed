...
; IMPORTANT NOTE: mov rsi, 0x00 would give us same result as xor rsi, rsi, but xor operation is faster (smaller instruction size and less CPU cycles)
        ...
        mov qword [heap_capacity], 0x201            ; setting heap_capacity to 512 bytes (2 * 16^2 = 512)
        ...
        mov rsi, [file_pointer]                     ; rsi = pointer to the file
        ...
        file_iteration_loop:                        ; we are iterating through every byte in the file
            ...
            mov bl, [rsi + rcx]                     ; bl is 8 lowest bytes from rbx register, and it holds the current byte we are reading
            ...
            inc rcx                                 ; after reading the byte, we increment the index
    ...
    ;CHECK IF WE NEED TO RESIZE HEAP
    ...
    move_up_the_heap:                       ;This needs to have rax = index of the element we want to move up
    ret

    move_down_the_heap:                     ;This needs to have rax = index of the element we want to move down
    ret
